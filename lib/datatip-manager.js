...
export class DataTipManager {
  ...
  initialize(renderer) {
    ...
    this.subscriptions.add(
      ...
      atom.config.observe(
        "atom-ide-datatip.showDataTipOnCursorMove",
        (toggleSwitch) => {
      ...
      atom.config.observe(
        "atom-ide-datatip.showDataTipOnMouseMove",
        (toggleSwitch) => {
    ...
    this.mouseMoveTimer = setTimeout(
      (evt) => {
        ...
        const distance = Math.abs(
          coordinates.mouse.left - coordinates.screen.left
        );
        ...
        const point = this.editor.bufferPositionForScreenPosition(
          screenPosition
        );
        if (
          this.currentMarkerRange === null ||
  ...
  onCommandEvt(evt) {
    ...
    if (atom.workspace.isTextEditor(editor)) {
      ...
      const isTooltipOpenForPosition =
  ...
  async showDataTip(editor, position, evt) {
    try {
      let datatip = null;
      for (const provider of this.providerRegistry.getAllProvidersForEditor(
        editor
      )) {
      ...
      if (!datatip) {
        ...
        if (
          this.currentMarkerRange != null &&
          datatip.range.intersectsWith(this.currentMarkerRange)
        ) {
          return;
        }
        ...
        if (datatip.component) {
          ...
          this.dataTipMarkerDisposables = this.mountDataTipWithMarker(
            editor,
            datatip.range,
            position,
            dataTipView
          ...
          const snippetHtml = await this.getSnippetHtml(
            datatip.markedStrings
              .filter((t) => t.type === "snippet")
              .map((t) => t.value),
            grammar
          );
          ...
          const dataTipView = new DataTipView({
            snippet: snippetHtml,
